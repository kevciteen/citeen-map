...
export async function GET(req: NextRequest) {
  ...
  // Avant : 9 requêtes séquentielles × ~40ms de latence Vercel↔Turso = ~360ms minimum
  // Après : 1 round-trip car Promise.all → ~80ms (lente seule la plus longue)
  ...
  const [
    ...
    // 1. Passoires F+G en pipeline actif
    db.get<{ n: number }>(`
    ...
    `),
    // 4. Taux conversion
    db.get<{ total: number; won: number }>(`
      SELECT
    ...
    db.all<{
      ...
      LIMIT 20
    `),
    // 6. Kanban count par stage
    db.all<{ stage: Stage; n: number }>(`
      SELECT stage, COUNT(*) AS n FROM prospects
      WHERE stage NOT IN ('won', 'lost')
      ${mineNoAlias}
      GROUP BY stage
    `),
    // 7. Sample prospects par stage actif
    db.all<{
      id: number;
      stage: Stage;
      label: string | null;
    ...
    db.all<{
      ...
      LIMIT 15
    `),
    // 10. Prospects pour la mini-carte
    db.all<{
      id: number;
      lat: number;
      lon: number;
